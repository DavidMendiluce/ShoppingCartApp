        ...
        <div class="row" style="padding-top: 10vh; display: flex; justify-content: space-around">
          <div class="col-md-3"  ng-repeat="product in products" style=" background-color: rgba(26,26,26,0.1); border: 1px solid rgba(159, 159, 159, 0.7); border-radius: 0.5vh; height: 55vh; margin:3vh 3vh;">
            <div class="imgContainer" style="background-color: white; position: relative; border: 1px solid rgba(159, 159, 159, 0.7); height: 35vh; top: 3vh;">
              <img ng-src="img/{{ product.image }}" class="img-responsive" width="250vh" height="250vh" style="padding: 3vh"/><br />
              <h4 class="text" style="padding-top: 10vh; color: rgba(26,26,26,0.8)">{{ product.name }}</h4>
              <h4 style="margin-bottom: 2vh;" class="text-danger">{{ product.price | currency : "€" }}</h4>
              <input style="font-size: 2vh; font-family: 'Nunito', sans-serif;color: #1a1a1a;background-color: rgba(51,220,251,0.7); border:rgba(51,220,251,0.7)" class="btn btn-success form-control" type="button" name="añadir" value="Añadir" ng-click="addtoCart(product)"/>
             </div>
          </div>
        </div>
      </form>
